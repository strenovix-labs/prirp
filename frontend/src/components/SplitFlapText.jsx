...
const SplitFlapText = ({
  ...
  // Clean, trimmed phrases to avoid dead spaces on shorter words
  const cleanPhrases = useMemo(
    () => phrases.map((p) => (fitContent ? p.trim() : (padTo ? p.padEnd(padTo, ' ') : p))),
    [phrases, fitContent, padTo]
  );

  const [tiles, setTiles] = useState(() => createTiles(cleanPhrases[0] || ''));
  ...
  useEffect(() => {
    ...
    const firstPhrase = cleanPhrases[0] || '';
    currentTextRef.current = firstPhrase;
    setTiles(createTiles(firstPhrase));

    if (cleanPhrases.length <= 1 || typeof window === 'undefined') {
    ...
    const animateTo = (targetPhrase) => {
      ...
      // Build target initial tiles
      const initialTargetTiles = targetChars.map((targetChar, index) => {
      ...
      const updateTiles = (updates) => {
        setTiles((previous) => {
          ...
          updates.forEach((update) => {
            if (update.index >= nextTiles.length) return;
    ...
    const scheduleNext = (delay) => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        if (nextIndex >= cleanPhrases.length && !loop) return;

        phraseIndex = nextIndex % cleanPhrases.length;
        const animationDuration = animateTo(cleanPhrases[phraseIndex]);
  ...
  }, [cleanPhrases, loop, cycleDelay, flipDuration, stagger, flipsPerChar, charset, prefersReducedMotion]);
  ...
  return (
    ...
    >
      {tiles.map((tile, index) => (
        <span className="split-flap-text__tile" aria-hidden="true" key={`${index}-${tile.tick}-${tile.current}`}>
